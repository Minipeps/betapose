...
SIXD = 'D:/Documents/Programmation/betapose/LineMod/test'
# SIXD = '/home/data/sixd/hinterstoisser/test'
DARKNET = '../data_linemod_gt'
# DARKNET = '/home/projects/detection/darknet/data_linemod_gt'
SEQS = ['%02d' % (i+1) for i in [1]]
GT_RATIO = 0.1
WIDTH = 640
HEIGHT = 480
    ...
    for idx, seq in enumerate(tbar):
        ...
        for img in tqdm(os.listdir(SIXD_IMGS), ascii=True, ncols=80):
            ...
            if random.random() < GT_RATIO:
                TRAIN_IMG_PATHS.append(dst_img)
            else:
                VALID_IMG_PATHS.append(dst_img)
        ...
        DARKNET_TRAIN_LIST = opj(DARKNET, seq, 'train.txt')
        DARKNET_VALID_LIST = opj(DARKNET, seq, 'valid.txt')
        ...
        with open(DARKNET_TRAIN_LIST, 'a') as f:
            for img_path in TRAIN_IMG_PATHS:
                f.write(img_path + '\n')
        with open(DARKNET_VALID_LIST, 'a') as f:
            for img_path in VALID_IMG_PATHS:
                f.write(img_path + '\n')
        with open(DARKNET_DATA_CFG, 'a') as f:
            f.write('classes = 1\n')
            f.write('train = ' + opj('..\data_linemod_gt', seq, 'train.txt') + '\n')
            f.write('valid = ' + opj('..\data_linemod_gt', seq, 'valid.txt') + '\n')
